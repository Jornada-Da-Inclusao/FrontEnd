...
const textStyle = { backgroundColor: 'transparent', border: 'none', padding: 0, margin: 0, fontSize: '1.1rem', fontFamily: 'sans-serif', textAlign: 'justify', color: 'var(--black, #000)' };
    ...
    <section className={styles.aboutContainer} id='sobre-nos'>
      ...
      <div className={styles.aboutContent}>
        <div className={styles.aboutItem}>
          ...
          />
          {(() => {
            const content = getAudioContent('sobre_plataforma_1');
            return content ? (
              <TextWithAudio text={content.text} audioUrl={content.audioUrl} textId={content.textId} textStyle={textStyle} />
            ) : (
        ...
        <div className={styles.aboutItem}>
          {(() => {
            const content = getAudioContent('sobre_plataforma_2');
            return content ? (
              <TextWithAudio text={content.text} audioUrl={content.audioUrl} textId={content.textId} textStyle={textStyle} />
            ) : (
              <p className="paragraph">
        ...
        <div className={styles.aboutItem}>
          ...
          />
          {(() => {
            const content = getAudioContent('sobre_plataforma_3');
            return content ? (
              <TextWithAudio text={content.text} audioUrl={content.audioUrl} textId={content.textId} textStyle={textStyle} />
            ) : (
              <p className="paragraph">
        ...
        <div className={styles.aboutItem}>
          {(() => {
            const content = getAudioContent('sobre_plataforma_4');
            return content ? (
              <TextWithAudio text={content.text} audioUrl={content.audioUrl} textId={content.textId} textStyle={textStyle} />
            ) : (
              <p className="paragraph">
