...
export default function JogoMemoria() {
  ...
  const checkForMatch = () => {
    ...
    if (
      optionOneId >= 0 &&
      optionTwoId >= 0 &&
      optionOneId < cards.length &&
      optionTwoId < cards.length
    ) {
      ...
      console.error(
        "IDs escolhidos estão fora do intervalo válido:",
        optionOneId,
        optionTwoId,
      );
    }
  ...
  const flipCard = (index) => {
    if (
      cardsChosenId.length < 2 &&
      !cardsChosenId.includes(index) &&
      !cardsWon.includes(index)
    ) {
    ...
    <>
      ...
      <div className={styles.gameContainer}>
        ...
        <div className={styles.resultContainer}>
          {" "}
          {/* Exibe o resultado de cartas combinadas */}
          <span className={styles.result}>
            Cartas combinadas: {cardsWon.length / 2}/{cards.length / 2}
          </span>{" "}
          {/* Exibe a quantidade de pares de cartas combinadas */}
        </div>
        <div className={styles.board}>
          {" "}
          {/* Tabuleiro onde as cartas serão exibidas */}
          {cards.map(
            (
              card,
              index, // Mapeia todas as cartas e exibe cada uma
            ) => (
              <img
                key={index}
                src={
                  cardsWon.includes(index) || cardsChosenId.includes(index)
                    ? card.img
                    : imgPlaceholder
                } // Exibe a imagem da carta se ela foi virada ou combinada, caso contrário, exibe a imagem de placeholder.
