...
export default function JogoVogais() {
        ...
        <div className={Style.dropArea}>
          ...
          {droppedLetras.map((letra) => (
            <div
              key={letra.id}
              id={"num" + letra.id}
              className={Style.letraInDroppable}
            >
              {String.fromCharCode(64 + letra.value)}{" "}
              {/* Converte o valor numérico da letra para o caractere correspondente. */}
            </div>
          ))}
        </div>
  ...
  const handleDragEnd = (event) => {
    ...
    if (over.id === "droppable-area") {
      ...
      if (
        letraToDrop &&
        !droppedLetras.some((letra) => letra.id === letraToDrop.id) &&
        (letraToDrop.id === 1 ||
          letraToDrop.id === 5 ||
          letraToDrop.id === 9 ||
          letraToDrop.id === 15 ||
        ...
        setLetras((prevLetras) =>
          prevLetras.filter((letra) => letra.id !== letraToDrop.id),
        );
      }
    }
  };
          ...
          <div className={Style.bodyGame}>
            <div className={Style.jogo}>
              <div className={Style.gameContent}>
                ...
                >
                  <DroppableArea />{" "}
                  {/* Exibe a área onde as letras podem ser soltas. */}
                  <CompJogoVogais Letras={letras} />{" "}
                  {/* Componente que renderiza as letras disponíveis para arrasto. */}
                </DndContext>
              </div>
            </div>
          </div>
